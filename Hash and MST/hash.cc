...
int insert(HashTable *hashTable, SLOT *slot){
	...
	if ( !hashTable->slot[hash] ){
		
		hashTable->slot[hash] = slot;
		hashTable->load++;
		return 0;
		...
		while (count < length){
			...
			if ( !hashTable->slot[count] ){
				
				hashTable->slot[count] = slot;
				hashTable->load++;
...
int search(HashTable *hashTable, int key){
	...
	if (hashTable->slot[hash] == NULL){
		
		reprobeCounter++;
		return reprobeCounter;
		
	}
	//printf("hashTable->slot[hash]->key\n");
	if ( hashTable->slot[hash]->key == key ){
		...
		while (count < length){
			...
			reprobeCounter++;
			//printf("hashTable->slot[hash]->key\n");
			if (hashTable->slot[count] == NULL){
		
				reprobeCounter++;
				return reprobeCounter;
				
			}
			if ( hashTable->slot[count]->key == key ){
		...
		return 0;	//renturn 0 means did not found 
	}
